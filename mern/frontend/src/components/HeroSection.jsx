import { ArrowRight, Star, Award, Users, Zap, Shield, Clock } from "lucide-react";
	...
	const getColoredText = () => {
		...
		for (let i = 0; i < lines.length; i++) {
			...
			if (currentIndex < text.length) {
				const visiblePart = text.substring(currentIndex, Math.min(endIndex, text.length));
				if (visiblePart) {
					result.push(
					...
					<div className='space-y-8'>
						<div>
							<h1 className='text-5xl lg:text-8xl font-bold leading-tight min-h-[200px] flex items-center overflow-hidden'>
								<span className="flex flex-wrap break-words max-w-full">
									{getColoredText()}
									<span className="animate-pulse text-red-500 ml-1 filter drop-shadow-lg">|</span>
								</span>
							</h1>
							<div className="relative">
								<p className='text-xl text-gray-300 mt-6 leading-relaxed opacity-0 animate-[fadeInUp_1s_ease-out_1.5s_forwards]'>
									...
									<span className="text-red-400 font-semibold"> flexible financing options</span>. 
									Over <span className="text-white font-bold">500+</span> satisfied customers and counting.
								</p>
						...
						<div className='flex items-center space-x-8 pt-8 opacity-0 animate-[fadeInUp_1s_ease-out_3s_forwards]'>
							<div className='flex items-center space-x-2 group cursor-pointer hover:scale-110 transition-all duration-300'>
								<Star className='text-yellow-400 group-hover:rotate-12 transition-transform duration-300' size={24} />
								<div>
									<p className='text-white font-semibold group-hover:text-yellow-400 transition-colors duration-300'>4.8/5</p>
									<p className='text-gray-400 text-sm'>Customer Rating</p>
								</div>
							</div>
							<div className='flex items-center space-x-2 group cursor-pointer hover:scale-110 transition-all duration-300'>
								<Award className='text-red-500 group-hover:rotate-12 transition-transform duration-300' size={24} />
								<div>
									<p className='text-white font-semibold group-hover:text-red-400 transition-colors duration-300'>5+ Years</p>
									<p className='text-gray-400 text-sm'>In Business</p>
								</div>
							</div>
							<div className='flex items-center space-x-2 group cursor-pointer hover:scale-110 transition-all duration-300'>
								<Users className='text-blue-400 group-hover:rotate-12 transition-transform duration-300' size={24} />
								<div>
									<p className='text-white font-semibold group-hover:text-blue-400 transition-colors duration-300'>500+</p>
